...
public class ClearMethPipe extends Item {
    public ClearMethPipe(Properties pProperties) {
        super(pProperties);
    }
    ...
    InteractionResultHolder<ItemStack> use(@NotNull Level worldIn, Player playerIn, @NotNull InteractionHand handIn) {
        ...
        playerIn.addEffect(new MobEffectInstance(RegistryHandler.METH_EFFECT.get(), 600, 2, false, true));
        if (!playerIn.isCreative()) {

            if(itemstack.is(RegistryHandler.GOLDEN_CLEAR_METH_PIPE.get())){

...
public class WeedEdible extends Item {
    public WeedEdible(Properties pProperties) {
        super(pProperties);
    }
    ...
    public @org.jetbrains.annotations.NotNull ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity livingEntity) {
        if (livingEntity instanceof ServerPlayer player) {
            ...
            player.addEffect(new MobEffectInstance(RegistryHandler.WEED_EFFECT.get(), 2500, 2, false, true));
            if(!isCreative){
                stack.shrink(1);
            }
        }
